...
- Report the query optimiser's plan for the queries `apexlog_list_slow_operations` ranked, as a `queryPlans` table of `{name, leadingOperationType, relativeCost, cardinality, sObjectCardinality}` rows. A slow query is a different fix depending on whether the optimiser can use an index, and nothing else in the response says which: above a `relativeCost` of 1 the query is not treated as selective. The worst plan is kept where one query was explained more than once. A plan is emitted at `DB,FINEST` alone, so the table is left out when none was recorded, and `dbLevel` beside it says whether one could have been ([#120])
