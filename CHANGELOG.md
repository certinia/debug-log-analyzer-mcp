...
- Reduce every tool response with no fact lost: `apexlog_list_slow_operations` by 4%, after the query plans it gained ([#86], [#108], [#120]) and `apexlog_execute_anonymous` by 30% ([#86], [#109]). `apexlog_get_summary` costs 16% more on a log that uses its limits, for the two tables it gained ([#62])
...
- Report the query optimiser's plan for the queries behind the rows `apexlog_list_slow_operations` returned, as a `queryPlans` table of `{name, leadingOperationType, relativeCost, cardinality, sObjectCardinality}` rows. A slow query is a different fix depending on whether the optimiser can use an index, and nothing else in the response says which: above a `relativeCost` of 1 the query is not treated as selective. One row per distinct query text, whether the row that carries it is one call, a folded group, or a namespace standing for several queries. The worst plan is kept where one query was explained more than once. A plan is emitted at `DB,FINEST` alone, so the table is left out when none was recorded, and `dbLevel` beside it says whether one could have been ([#120])
...
- Report progress from `apexlog_execute_anonymous` while it connects, sets the trace flag, executes and writes, so a client can say what a long run is doing. Sent only to a client that asked for progress ([#65])
...
- Return the debug log of the run that produced it. `apexlog_execute_anonymous` took the newest `ApexLog` row for the user, so a second process logging against the same user could hand back its log instead. The log now comes back with the result, over the SOAP Apex API and a debug header, and the org is queried only to name the file the way `sf` names it — a query that finds nothing costs a filename and nothing else, and the file is named `apex-<epoch>.log` ([#65])
