...
- Reduce the standing cost of having the server connected by 24%: `apexlog_execute_anonymous` by 50%, `apexlog_list_limit_risks` by 28% and `apexlog_get_summary` by 5% ([#87], [#108], [#103]). `apexlog_list_slow_operations` costs 55% more, for the five parameters that select what it ranks, for what `groupBy` now states about the grouped row, and for what `lineNumber` points at ([#108], [#101], [#126], [#127], [#136])
...
- Say what `lineNumber` points at on an `apexlog_list_slow_operations` row: the line the call was made on, in the calling file, and never a line in the code that ran. An agent that read it as the callee's line opened the wrong file — which is the common case, because the rows it matters most on name nothing openable, such as `DML Op:Insert Type:Account` or `List<Account>.add(Object)`. It is null on a kind with no Apex call site: a code unit, a flow, a workflow or a managed package boundary ([#136])
...
[#136]: https://github.com/certinia/debug-log-analyzer-mcp/issues/136
