...
- Reduce every tool response with no fact lost: `apexlog_list_slow_operations` by 4%, after the query plans it gained ([#86], [#108], [#120]) and `apexlog_execute_anonymous` by 30% ([#86], [#109]). `apexlog_get_summary` costs 17% more on a log that uses its limits, for the two tables it gained ([#62], [#97])
