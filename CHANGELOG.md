...
- Tell a 2026-07-28 client that `tools/list` keeps for an hour and that a shared cache may hold one copy of it. The definitions are fixed for the life of the process and hold nothing about the caller. Without the hint the SDK states the conservative default of no caching, so every client re-reads the definitions on every turn ([#94])
