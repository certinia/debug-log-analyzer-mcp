...
`sortBy: "heapSelfNetBytes"` ranks by the net heap each row's own code kept, and adds that column. Use it when a transaction dies on heap rather than CPU — time alone cannot tell code that holds memory from code that churns and frees it.
...
| `sortBy`        | string   | No       | Rank on `durationSelfMs` (default) or `heapSelfNetBytes`, which adds that column                         |
