...
import { parse, ApexLog, LogLine } from '../src/ApexLogParser.js';
...
describe('analyzeLogPerformance', () => {
  ...
  describe('Tool Configuration', () => {
    it('should have correct tool configuration', () => {
      expect(analyzeLogPerformanceTool.name).toBe('analyze_apex_log_performance');
      expect(analyzeLogPerformanceTool.description).toContain('Analyze an Apex debug log file');
      expect(analyzeLogPerformanceTool.inputSchema.required).toEqual(['logFilePath']);
      expect(analyzeLogPerformanceTool.inputSchema.properties.logFilePath.type).toBe('string');
      expect(analyzeLogPerformanceTool.inputSchema.properties.topMethods.default).toBe(10);
      expect(analyzeLogPerformanceTool.inputSchema.properties.minDuration.default).toBe(0);
    });
  });
  ...
  describe('File Validation', () => {
    ...
    it('should proceed when file exists', async () => {
      ...
      expect(mockedFs.readFile).toHaveBeenCalledWith('/valid/file.log', 'utf-8');
  ...
  describe('Basic Functionality', () => {
    it('should analyze log with default parameters', async () => {
      ...
      const result = await analyzeLogPerformance(args);
      const parsedResult: LogAnalysisResult = JSON.parse(result.content[0].text);
    ...
    it('should limit results with topMethods parameter', async () => {
      const args: AnalyzeLogArgs = { logFilePath: '/test/file.log', topMethods: 2 };
      const mockApexLog = createMockApexLog();
    ...
    it('should filter by minimum duration', async () => {
      ...
      expect(parsedResult.slowestMethods).toHaveLength(2);
      expect(parsedResult.slowestMethods.every(method => method.duration >= 300000000)).toBe(true);
    });
  ...
  describe('extractMethods function', () => {
    ...
    it('should filter methods by minimum duration', () => {
      ...
      expect(methods.every(method => method.duration >= 300000000)).toBe(true);
    });
  ...
  describe('Performance Summary Generation', () => {
    ...
    it('should generate appropriate summary for no methods', async () => {
      const args: AnalyzeLogArgs = { logFilePath: '/test/file.log', minDuration: 2000000000 };
      const mockApexLog = createMockApexLog();
      ...
      expect(parsedResult.summary).toBe('No methods found matching the criteria.');
  ...
  describe('Recommendations Generation', () => {
    it('should generate SOQL query recommendations', async () => {
      ...
      const soqlRecommendation = parsedResult.recommendations.find(rec =>
    ...
    it('should generate DML recommendations', async () => {
      ...
      const dmlRecommendation = parsedResult.recommendations.find(rec =>
        rec.includes('DML operations') && rec.includes('bulkifying')
      );
    ...
    it('should generate SOQL rows recommendations', async () => {
      ...
      const rowsRecommendation = parsedResult.recommendations.find(rec =>
        rec.includes('SOQL rows') && rec.includes('WHERE clauses')
      );
    ...
    it('should generate high percentage recommendations', async () => {
      ...
      const percentageRecommendation = parsedResult.recommendations.find(rec =>
        rec.includes('% of total execution time') && rec.includes('priority for optimization')
      );
    ...
    it('should only analyze top 3 methods for recommendations', async () => {
      const args: AnalyzeLogArgs = { logFilePath: '/test/file.log', topMethods: 10 };
      ...
      const methodMentions = parsedResult.recommendations.filter(rec =>
        rec.includes('Method1') || rec.includes('Method2') || rec.includes('Method3') || rec.includes('Method4')
      );

      const mentionsMethod4 = methodMentions.some(rec => rec.includes('Method4'));
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle parsing errors gracefully', async () => {
      ...
      await expect(analyzeLogPerformance(args)).rejects.toThrow('Parsing failed');
    });
    ...
    it('should handle file read errors', async () => {
      ...
      await expect(analyzeLogPerformance(args)).rejects.toThrow('Permission denied');
    });
  });
  ...
  describe('Interface Contracts', () => {
    it('should return SlowMethod objects with correct structure', async () => {
      ...
      expect(['number', 'string', 'object']).toContain(typeof method.lineNumber);
    });
  ...
  function createMockApexLog(): any {
    const slowMethod = createMockLogLine('SlowMethod', 500000000, 500000000, 'default', 1, 5, 10, 100, 1000);
    const mediumMethod = createMockLogLine('MediumMethod', 400000000, 400000000, 'default', 2, 2, 3, 50, 200);
    const fastMethod = createMockLogLine('FastMethod', 100000000, 100000000, 'default', 3, 1, 1, 10, 50);
  ...
  function createMockApexLogWithNamespaces(): any {
    const defaultMethod = createMockLogLine('DefaultMethod', 500000000, 500000000, 'default', 1);
    const customMethod = createMockLogLine('CustomMethod', 400000000, 400000000, 'CustomNamespace', 2);
  ...
  function createMockApexLogWithHighSOQL(): any {
    const highSOQLMethod = createMockLogLine('HighSOQLMethod', 500000000, 500000000, 'default', 1, 2, 8, 50, 500);
  ...
  function createMockApexLogWithHighDML(): any {
    const highDMLMethod = createMockLogLine('HighDMLMethod', 500000000, 500000000, 'default', 1, 6, 2, 100, 50);
  ...
  function createMockApexLogWithHighSOQLRows(): any {
    const highRowsMethod = createMockLogLine('HighRowsMethod', 500000000, 500000000, 'default', 1, 1, 2, 10, 1500);
  ...
  function createMockApexLogWithHighPercentage(): any {
    const highPercentageMethod = createMockLogLine('HighPercentageMethod', 300000000, 300000000, 'default', 1, 1, 1, 10, 50);
  ...
  function createMockApexLogWithGoodPerformance(): any {
    const goodMethod = createMockLogLine('GoodMethod', 100000000, 100000000, 'default', 1, 1, 2, 50, 100);
  ...
  function createMockApexLogWithManyMethods(): any {
    const method1 = createMockLogLine('Method1', 400000000, 400000000, 'default', 1, 8, 8, 100, 1200);
    const method2 = createMockLogLine('Method2', 300000000, 300000000, 'default', 2, 6, 6, 80, 1000);
    const method3 = createMockLogLine('Method3', 200000000, 200000000, 'default', 3, 4, 4, 60, 800);
    const method4 = createMockLogLine('Method4', 100000000, 100000000, 'default', 4, 8, 8, 100, 1200); // Should not appear in recommendations
