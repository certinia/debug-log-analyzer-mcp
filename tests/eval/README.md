...
| `minimal.log` | A `System.debug('')` run: the smallest transaction that still emits a limit block, with `DATA_ACCESS` cut from its header. | Everything is zero. This is the fixture that fails if a zero is ever omitted instead of reported, because "no DML statements ran" has to be answerable from the payload. Its header states no `DATA_ACCESS` level, as 122 of 123 real logs do not, so it is also the only fixture that reaches an empty `categories.level`. |
